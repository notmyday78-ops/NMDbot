...
export class ModLogService {
  ...
  async getSetting(guildId, category) {
    const settings = await this.getSettings(guildId);
    return settings.get(category) ?? null;
  }
  async setChannel(guildId, category, channelId, enabled = true) {
  ...
  async remove(guildId, category) {
    const removed = await modLogRepository.delete(guildId, category);
    this.invalidateCache(guildId);
    return removed;
  }
  async sendLog(guild, category, payload) {
  ...
  async sendDirect(guild, channelId, payload) {
