...
function loadListDefinitions(configPath) {
  try {
    ...
    Object.entries(commandGroups).forEach(([key, value]) => {
      ...
      const entries = Object.entries(value.items).filter(
      ...
      const title = typeof value.title === "string" && value.title.trim().length > 0 ? value.title.trim() : formatTitleFromKey(key);
      const description = typeof value.description === "string" && value.description.trim().length > 0 ? value.description.trim() : void 0;
    ...
    if (isRecord(data.help)) {
      const helpPrefix = typeof data.help.prefix === "string" ? data.help.prefix.trim().toLowerCase() : "";
      const trigger = typeof data.help.prefix === "string" ? data.help.prefix.trim() : "";
      if (helpPrefix && trigger) {
        const helpDescription = typeof data.help.description === "string" && data.help.description.trim().length > 0 ? data.help.description.trim() : void 0;
        const helpCommands = Array.isArray(
          data.help.commands
        ) ? data.help.commands.reduce(
          (acc, command) => {
            if (!isRecord(command)) {
              return acc;
            }
            ...
            const cmdDescription = typeof command.description === "string" && command.description.trim().length > 0 ? command.description.trim() : void 0;
...
export class ListCommandService {
  ...
  async handle(message) {
    ...
    const embed = new EmbedBuilder().setColor(5793266).setTimestamp();
    if (definition.kind === "list") {
      const entriesDescription = definition.entries.map((entry) => `\u2022 [${entry.name}](${entry.url})`).join("\n");
      const description = definition.description ? `${definition.description}

${entriesDescription}` : entriesDescription;
