...
export class JTCService {
  async setupJTC(guild, baseVoiceChannelId, categoryId, panelChannelId, channelNameFormat) {
    const configData = {
      baseVoiceChannelId,
      categoryId,
      panelChannelId,
  ...
  async sendOrUpdatePanel(guild) {
    ...
    const embed = new EmbedBuilder().setTitle(t("jtc.panel.title")).setDescription(t("jtc.panel.description")).setColor(5793266);
    const lockButton = new ButtonBuilder().setCustomId("jtc_lock").setLabel(t("jtc.buttons.lock")).setEmoji("\u{1F512}").setStyle(ButtonStyle.Primary);
    const unlockButton = new ButtonBuilder().setCustomId("jtc_unlock").setLabel(t("jtc.buttons.unlock")).setEmoji("\u{1F513}").setStyle(ButtonStyle.Secondary);
    const renameButton = new ButtonBuilder().setCustomId("jtc_rename").setLabel(t("jtc.buttons.rename")).setEmoji("\u270F\uFE0F").setStyle(ButtonStyle.Success);
    const claimButton = new ButtonBuilder().setCustomId("jtc_claim").setLabel(t("jtc.buttons.claim")).setEmoji("\u{1F451}").setStyle(ButtonStyle.Danger);
    ...
    const selectMenu = new StringSelectMenuBuilder().setCustomId("jtc_limit").setPlaceholder(t("jtc.menu.limitPlaceholder")).addOptions([
      { label: t("jtc.menu.noLimit"), value: "0", emoji: "\u267E\uFE0F" },
      { label: t("jtc.menu.users", { count: 2 }), value: "2", emoji: "\u{1F465}" },
      { label: t("jtc.menu.users", { count: 3 }), value: "3", emoji: "\u{1F465}" },
      { label: t("jtc.menu.users", { count: 4 }), value: "4", emoji: "\u{1F465}" },
      { label: t("jtc.menu.users", { count: 5 }), value: "5", emoji: "\u{1F465}" },
      { label: t("jtc.menu.users", { count: 10 }), value: "10", emoji: "\u{1F465}" },
      { label: t("jtc.menu.users", { count: 15 }), value: "15", emoji: "\u{1F465}" },
      { label: t("jtc.menu.users", { count: 20 }), value: "20", emoji: "\u{1F465}" },
      { label: t("jtc.menu.users", { count: 25 }), value: "25", emoji: "\u{1F465}" }
    ]);
  ...
  async getTempChannelForInteraction(interaction, requireOwner = true) {
  ...
  async handleRenameModal(interaction) {
    ...
    if (!data) return;
    try {
      const modal = new ModalBuilder().setCustomId("jtc_rename_modal").setTitle(t("jtc.modal.renameTitle"));
      const nameInput = new TextInputBuilder().setCustomId("channel_name").setLabel(t("jtc.modal.newNameLabel")).setStyle(TextInputStyle.Short).setMinLength(1).setMaxLength(100).setRequired(true).setValue(data.voiceChannel.name);
  ...
  async handleUserLimit(interaction) {
    const data = await this.getTempChannelForInteraction(interaction, true);
    if (!data) return;
    try {
