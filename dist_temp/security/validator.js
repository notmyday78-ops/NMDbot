...
export const ValidationSchemas = {
  ...
  description: z.string().max(4e3).trim().optional(),
  shortText: z.string().max(1e3).trim(),
  // Numbers
  level: z.number().int().min(1).max(10),
  amount: z.number().int().min(0).max(1e9),
  // Max 1 billion
  percentage: z.number().min(0).max(100),
  // Time
  duration: z.number().int().min(1).max(31536e3),
...
export const CommandSchemas = {
  ...
  ticket: {
    panel: z.object({
      ...
      autoClose: z.number().int().min(0).max(604800).optional()
      // Max 7 days
    }),
    close: z.object({
  ...
  config: {
    ...
    xp: z.object({}).optional(),
    // No input required - just shows config UI
    eco: z.object({}).optional(),
    // No input required - just shows config UI
    welcome: z.object({}).optional(),
    // No input required - just shows config UI
    autorole: z.object({}).optional(),
    // No input required - just shows config UI
    goodbye: z.object({}).optional()
    // No input required - just shows config UI
  }
};
export class Validator {
   ...
   */
  static validatePermissions(required, userPerms) {
   ...
   */
  static isFileSafe(filename, mimeType) {
   ...
   */
  static isRegexSafe(pattern, maxLength = 100) {
  ...
  static containsNestedQuantifiers(pattern) {
    const quantifierStack = [];
    for (let i = 0; i < pattern.length; i++) {
      ...
      if (char === "{") {
        ...
        if (parts.length === 0 || parts.length > 2 || parts.some((part) => part === "" || Number.isNaN(Number(part)))) {
          return true;
        }
  ...
  static hasExcessiveWildcards(pattern) {
