...
export class Sanitizer {
  ...
  static MASS_MENTION_THRESHOLD = 5;
  // Dangerous patterns
  ...
  static escapeMentions(text) {
    return text.replace(/@everyone/g, "@\u200Beveryone").replace(/@here/g, "@\u200Bhere").replace(/<@!?(\d+)>/g, "<@\u200B$1>").replace(/<@&(\d+)>/g, "<@\u200B&$1>").replace(/<#(\d+)>/g, "<#\u200B$1>");
  }
  /**
   * Escape links to prevent embeds
   */
  static escapeLinks(text) {
  ...
  static hasMassMentions(text) {
    const mentionCount = (text.match(/<@!?\d+>/g) || []).length + (text.match(/<@&\d+>/g) || []).length + (text.includes("@everyone") ? 10 : 0) + (text.includes("@here") ? 5 : 0);
    return mentionCount > this.MASS_MENTION_THRESHOLD;
  }
  /**
   * Remove sensitive information
   */
  static removeSensitive(text) {
    return text.replace(this.DANGEROUS_PATTERNS.token, "[TOKEN_REMOVED]").replace(this.DANGEROUS_PATTERNS.webhook, "[WEBHOOK_REMOVED]").replace(this.DANGEROUS_PATTERNS.ipAddress, "[IP_REMOVED]");
  }
  /**
   * Sanitize code block content
   */
  static sanitizeCodeBlock(code, language) {
    ...
    const lang = language && validLanguages.includes(language.toLowerCase()) ? language.toLowerCase() : "";
    return `\`\`\`${lang}
${sanitized}
\`\`\``;
  }
   ...
   */
  static createPreview(text, maxLength = 100) {
