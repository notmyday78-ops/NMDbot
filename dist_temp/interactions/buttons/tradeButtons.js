...
export async function handleTradeButtons(interaction) {
  ...
  if (action === "offer" && subAction === "coins") {
    const modal = new ModalBuilder().setCustomId(`trade_offer_coins_modal_${tradeId}`).setTitle("Offer Coins");
    const amountInput = new TextInputBuilder().setCustomId("amount").setLabel("Amount of coins to offer").setStyle(TextInputStyle.Short).setRequired(true);
  ...
  if (action === "offer" && subAction === "item") {
    const modal = new ModalBuilder().setCustomId(`trade_offer_item_modal_${tradeId}`).setTitle("Offer Item");
    const itemNameInput = new TextInputBuilder().setCustomId("itemName").setLabel("Item Name").setStyle(TextInputStyle.Short).setRequired(true);
    const quantityInput = new TextInputBuilder().setCustomId("quantity").setLabel("Quantity").setStyle(TextInputStyle.Short).setValue("1").setRequired(true);
  ...
  if (action === "accept") {
    ...
    try {
      await db.transaction(async (tx) => {
        await tx.update(economyTrades).set({ status: "completed", receiverId: interaction.user.id }).where(eq(economyTrades.id, tradeId));
        const initiatorOfferCoins = trade.initiatorOffer?.coins || 0;
        const receiverOfferCoins = trade.receiverOffer?.coins || 0;
        const initiatorOfferItems = trade.initiatorOffer?.items || [];
        const receiverOfferItems = trade.receiverOffer?.items || [];
        const [initiatorBalance] = await tx.select().from(economyBalances).where(
          and(
            eq(economyBalances.userId, trade.initiatorId),
            eq(economyBalances.guildId, trade.guildId)
          )
        ).limit(1);
        const [receiverBalance] = await tx.select().from(economyBalances).where(
          and(
        ...
        if (initiatorOfferCoins > 0) {
          await tx.update(economyBalances).set({ balance: initiatorBalance.balance - initiatorOfferCoins }).where(
            and(eq(economyBalances.userId, trade.initiatorId), eq(economyBalances.guildId, trade.guildId))
          );
          if (receiverBalance) {
            await tx.update(economyBalances).set({ balance: receiverBalance.balance + initiatorOfferCoins }).where(
        ...
        if (receiverOfferCoins > 0) {
          await tx.update(economyBalances).set({ balance: receiverBalance.balance - receiverOfferCoins }).where(
            and(eq(economyBalances.userId, interaction.user.id), eq(economyBalances.guildId, trade.guildId))
          );
          if (initiatorBalance) {
            await tx.update(economyBalances).set({ balance: initiatorBalance.balance + receiverOfferCoins }).where(
              and(eq(economyBalances.userId, trade.initiatorId), eq(economyBalances.guildId, trade.guildId))
            );
          }
        }
        const processItems = async (fromUserId, toUserId, items) => {
          for (const item of items) {
            const itemName = item.name;
            const quantity = item.quantity;
            const [shopItem] = await tx.select().from(economyShopItems).where(and(eq(economyShopItems.guildId, trade.guildId), eq(economyShopItems.name, itemName))).limit(1);
            if (!shopItem || !shopItem.tradeable) throw new Error(`Item ${itemName} is not tradeable.`);
            const [userItem] = await tx.select().from(economyUserItems).where(and(
            ...
            if (userItem.quantity === quantity) {
              ...
              await tx.update(economyUserItems).set({ quantity: userItem.quantity - quantity }).where(eq(economyUserItems.id, userItem.id));
            }
            const [receiverItem] = await tx.select().from(economyUserItems).where(and(
            ...
            if (receiverItem) {
              await tx.update(economyUserItems).set({ quantity: receiverItem.quantity + quantity }).where(eq(economyUserItems.id, receiverItem.id));
            } else {
...
export async function handleTradeModal(interaction) {
  ...
  if (!isInitiator && !isReceiver && trade.receiverId === null) {
    trade.receiverId = interaction.user.id;
    isReceiver = true;
    await db.update(economyTrades).set({ receiverId: interaction.user.id }).where(eq(economyTrades.id, tradeId));
  } else if (!isInitiator && !isReceiver) {
    await interaction.reply({ content: "You are not part of this trade.", ephemeral: true });
    return;
  }
  const currentOffer = trade[offerKey] || { coins: 0, items: [] };
  if (interaction.customId.startsWith("trade_offer_coins_modal_")) {
    ...
    const [balance] = await db.select().from(economyBalances).where(
      and(
        eq(economyBalances.userId, interaction.user.id),
        eq(economyBalances.guildId, trade.guildId)
      )
    ...
    const [shopItem] = await db.select().from(economyShopItems).where(and(eq(economyShopItems.guildId, trade.guildId), eq(economyShopItems.name, itemName))).limit(1);
    if (!shopItem) {
    ...
    const [userItem] = await db.select().from(economyUserItems).where(and(
    ...
    const existingItem = currentOffer.items.find((i) => i.name === itemName);
    if (existingItem) {
  ...
  const [newTrade] = await db.select().from(economyTrades).where(eq(economyTrades.id, tradeId)).limit(1);
  if (!newTrade) return;
  const initCoins = newTrade.initiatorOffer?.coins || 0;
  const initItems = (newTrade.initiatorOffer?.items || []).map((i) => `${i.quantity}x ${i.name}`).join(", ") || "None";
  const recCoins = newTrade.receiverOffer?.coins || 0;
  const recItems = (newTrade.receiverOffer?.items || []).map((i) => `${i.quantity}x ${i.name}`).join(", ") || "None";
  const receiverDisplay = newTrade.receiverId ? `<@${newTrade.receiverId}>` : "Anyone";
  const embed = new EmbedBuilder().setTitle(newTrade.receiverId ? "Trade Initiated" : "Open Trade").setDescription(`<@${newTrade.initiatorId}> wants to trade with ${receiverDisplay}`).addFields(
    {
...
Items: ${recItems}`,
  ...
  ).setColor(3447003).setFooter({ text: "Trade updates in real-time" });
