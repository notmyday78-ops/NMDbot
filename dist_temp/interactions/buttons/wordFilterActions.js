...
export async function handleWordFilterActionButtons(interaction) {
  ...
  const rule = Number.isFinite(ruleId) ? await wordFilterService.getRule(interaction.guildId, ruleId) : null;
  if (!rule) {
    await interaction.reply({
...
async function handleWarn(interaction, targetUserId, rule) {
...
async function handleTimeout(interaction, targetUserId, rule, durationSeconds) {
  const guild = interaction.guild;
...
async function handleKick(interaction, targetUserId, rule) {
  const guild = interaction.guild;
  const member = await guild.members.fetch(targetUserId).catch(() => null);
  if (!member) {
    await interaction.editReply({
...
async function handleBan(interaction, targetUserId, rule) {
  const guild = interaction.guild;
  if (!await ensureModerationPermissions(
    interaction,
...
async function ensureModerationPermissions(interaction, targetMember, requiredPermission) {
  ...
  if (executorMember && targetMember.roles.highest.comparePositionTo(executorMember.roles.highest) >= 0) {
    await interaction.editReply({
...
async function appendResolution(interaction, resolution) {
