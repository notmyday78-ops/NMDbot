...
import { t } from "../../i18n";
export async function handleWarningModals(interaction) {
...
async function handleWarnEdit(interaction, warnId) {
...
async function handleAutomationCreate(interaction, triggerTypeParam, triggerValueParam, notifyChannelParam) {
  await interaction.deferReply({ ephemeral: true });
  ...
  if (!triggerType || triggerValue === void 0 || Number.isNaN(triggerValue) || triggerValue < 1) {
    await interaction.editReply({
...
function getOptionalField(interaction, fieldId) {
  try {
...
const parseTriggerType = (value) => {
  if (!value) {
    return void 0;
  }
  return value === "warn_count" || value === "warn_level" ? value : void 0;
};
...
const parseChannelIdParam = (value) => {
  if (!value || value === "none") {
    return null;
  }
  return value;
...
const normalizeActionChoice = (value) => value.toLowerCase().replace(/\s+/g, " ").trim();
const mapAutomationAction = (actionRaw, messageRaw) => {
