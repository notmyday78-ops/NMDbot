...
export const data = new SlashCommandBuilder().setName("starboard").setDescription("Configure the starboard system").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).addSubcommand(
  (subcommand) => subcommand.setName("setup").setDescription("Enable and setup starboard channel").addChannelOption(
    (option) => option.setName("channel").setDescription("The starboard channel").setRequired(true).addChannelTypes(ChannelType.GuildText)
  )
).addSubcommand((subcommand) => subcommand.setName("disable").setDescription("Disable starboard")).addSubcommand(
  (subcommand) => subcommand.setName("threshold").setDescription("Set the number of stars required").addIntegerOption(
    (option) => option.setName("count").setDescription("Number of stars").setRequired(true).setMinValue(1)
  )
).addSubcommand(
  (subcommand) => subcommand.setName("emoji").setDescription("Set the starboard emoji").addStringOption(
    (option) => option.setName("emoji").setDescription("Emoji to use (e.g., \u2B50)").setRequired(true)
  )
).addSubcommand(
  (subcommand) => subcommand.setName("info").setDescription("View current starboard settings")
);
...
export async function execute(interaction) {
  ...
  try {
    if (subcommand === "setup") {
      ...
      const embed = new EmbedBuilder().setColor(16755763).setTitle("Starboard Configuration").addFields(
