...
export const data = new SlashCommandBuilder().setName("shop").setDescription(
  t("commands.economy.subcommands.shop.description", {
    defaultValue: "View and purchase items from the shop"
  })
).setDescriptionLocalizations(createLocalizationMap(subcommandDescriptions.economy.shop.group)).addSubcommand(
  (subcommand) => subcommand.setName("view").setDescription(
  ...
  (subcommand) => subcommand.setName("buy").setDescription(
    t("commands.economy.subcommands.shop.buy.description", {
      defaultValue: "Purchase an item from the shop"
    })
  ).setDescriptionLocalizations(createLocalizationMap(subcommandDescriptions.economy.shop.buy)).addStringOption(
    ...
    ).setDescriptionLocalizations(createLocalizationMap(optionDescriptions.buyItem)).setRequired(true).setAutocomplete(true)
  ).addIntegerOption(
    (option) => option.setName("quantity").setDescription(
      t("commands.economy.subcommands.shop.buy.options.quantity", {
        defaultValue: "Quantity to purchase"
      })
    ).setDescriptionLocalizations(createLocalizationMap(optionDescriptions.buyQuantity)).setMinValue(1).setMaxValue(99)
  )
).addSubcommand(
  (subcommand) => subcommand.setName("inventory").setDescription(
...
**${t("commands.economy.shop.view.price", { lng: locale })}:** ${settings.currencySymbol}${item.price.toLocaleString()} | **${t("commands.economy.shop.view.stock", { lng: locale })}:** ${stockText}${item.effectType ? `
**${t("commands.economy.shop.view.effect", { lng: locale })}:** ${formatEffect(item.effectType, item.effectValue, locale)}` : ""}`,
    ...
    const createButtons = (page) => {
      return new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("shop_first").setLabel("\u23EE\uFE0F").setStyle(ButtonStyle.Secondary).setDisabled(page === 0),
        new ButtonBuilder().setCustomId("shop_prev").setLabel("\u25C0\uFE0F").setStyle(ButtonStyle.Secondary).setDisabled(page === 0),
        new ButtonBuilder().setCustomId("shop_refresh").setLabel("\u{1F504}").setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId("shop_next").setLabel("\u25B6\uFE0F").setStyle(ButtonStyle.Secondary).setDisabled(page === totalPages - 1),
        new ButtonBuilder().setCustomId("shop_last").setLabel("\u23ED\uFE0F").setStyle(ButtonStyle.Secondary).setDisabled(page === totalPages - 1)
      );
    };
...
async function handleBuyItem(interaction) {
  ...
  try {
    ...
    const embed = new EmbedBuilder().setTitle(t("commands.economy.shop.buy.success", { lng: locale })).setDescription(
      t("commands.economy.shop.buy.purchased", { lng: locale, quantity, item: item.name })
    ).setColor(3066993).setThumbnail(interaction.user.displayAvatarURL()).addFields(
      {
...
async function handleViewInventory(interaction) {
  ...
  try {
    ...
    const embed = new EmbedBuilder().setTitle(t("commands.economy.shop.inventory.title", { lng: locale })).setDescription(
      t("commands.economy.shop.inventory.itemCount", {
        lng: locale,
        count: userItems.length,
        s: userItems.length > 1 ? "s" : ""
      })
    ).setColor(3447003).setThumbnail(interaction.user.displayAvatarURL()).setTimestamp();
    userItems.forEach((userItem) => {
      const expiryText = userItem.expiresAt ? `
${t("commands.economy.shop.inventory.expires", { lng: locale })}: <t:${Math.floor(userItem.expiresAt.getTime() / 1e3)}:R>` : "";
      embed.addFields({
        name: `${userItem.item.name} (x${userItem.quantity})`,
        value: `${userItem.item.description}${userItem.item.effectType ? `
**${t("commands.economy.shop.inventory.effect", { lng: locale })}:** ${formatEffect(userItem.item.effectType, userItem.item.effectValue, locale)}` : ""}${expiryText}`,
        inline: false
      });
    });
    await interaction.editReply({ embeds: [embed] });
...
export async function autocomplete(interaction) {
  ...
  try {
    const items = await economyRepository.getShopItems(guildId, true);
    const filtered = items.filter((item) => item.name.toLowerCase().includes(focusedValue)).slice(0, 25);
...
function formatEffect(effectType, effectValue, locale) {
  switch (effectType) {
    case "rob_protection":
...
async function createDefaultShopItems(guildId) {
  const defaultItems = [
    ...
    },
    {
      ...
      type: "booster",
      effectType: "xp_boost",
      effectValue: { multiplier: 2, duration: 3600 },
      // 1 hour
      stock: -1
    },
    {
      ...
      effectValue: { bonus: 10, duration: 7200 },
      // 2 hours
      stock: -1
    },
    {
      ...
      effectValue: { multiplier: 1.5, duration: 10800 },
      // 3 hours
      stock: -1
    },
    {
