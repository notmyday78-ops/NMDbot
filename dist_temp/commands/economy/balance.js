...
export const data = new SlashCommandBuilder().setName("balance").setDescription(
...
export async function execute(interaction) {
  ...
  try {
    ...
    const embed = new EmbedBuilder().setTitle(
    ...
    ).setColor(3066993).setThumbnail(targetUser.displayAvatarURL()).addFields(
      {
      ...
      },
      {
        ...
        value: `${t("commands.economy.subcommands.balance.embed.totalEarned", { defaultValue: "Total Earned", lng: locale })}: ${settings.currencySymbol} ${balance.totalEarned.toLocaleString()}
...
${t("commands.economy.subcommands.balance.embed.totalGambled", { defaultValue: "Total Gambled", lng: locale })}: ${settings.currencySymbol} ${balance.totalGambled.toLocaleString()}`,
    ...
    if (transactions.length > 0) {
      const transactionList = transactions.map((tObj) => {
        const prefix = tObj.amount > 0 ? "+" : "";
        const emoji = tObj.amount > 0 ? "\u{1F4C8}" : "\u{1F4C9}";
