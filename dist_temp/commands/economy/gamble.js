...
export const data = new SlashCommandBuilder().setName("gamble").setDescription(
  t("commands.economy.subcommands.gamble.description", {
    defaultValue: "Play various gambling games"
  })
).setDescriptionLocalizations(createLocalizationMap(subcommandDescriptions.economy.gamble.group)).addSubcommand(
  (subcommand) => subcommand.setName("dice").setDescription(
    ...
    (option) => option.setName("bet").setDescription(
      t("commands.economy.subcommands.gamble.options.bet", { defaultValue: "Amount to bet" })
    ).setDescriptionLocalizations(createLocalizationMap(optionDescriptions.bet)).setRequired(true).setMinValue(1)
  )
).addSubcommand(
  (subcommand) => subcommand.setName("coinflip").setDescription(
    ...
    (option) => option.setName("choice").setDescription(
      t("commands.economy.subcommands.gamble.coinflip.options.choice", {
        defaultValue: "Heads or tails"
      })
    ).setDescriptionLocalizations(createLocalizationMap(optionDescriptions.coinflipChoice)).setRequired(true).addChoices(
      {
        name: "Heads",
        value: "heads",
  ...
  (subcommand) => subcommand.setName("slots").setDescription(
  ...
  (subcommand) => subcommand.setName("blackjack").setDescription(
  ...
  (subcommand) => subcommand.setName("roulette").setDescription(
    ...
    ).setDescriptionLocalizations(createLocalizationMap(optionDescriptions.rouletteType)).setRequired(true).addChoices(
      {
        name: "Red",
        value: "color:red",
    ...
    (option) => option.setName("number").setDescription(
      t("commands.economy.subcommands.gamble.roulette.options.number", {
        defaultValue: "Specific number to bet on (0-36)"
      })
    ).setDescriptionLocalizations(createLocalizationMap(optionDescriptions.rouletteNumber)).setMinValue(0).setMaxValue(36)
  )
);
...
export async function execute(interaction) {
  ...
  try {
    ...
    const recentGambles = await economyRepository.getRecentGambles(userId, guildId, 3600);
    if (recentGambles >= 10) {
    ...
    const userMessage = error instanceof RateLimitError ? error.message : t("commands.economy.gamble.error", {
...
function createDiceEmbed(result, settings, locale) {
  const details = result.details;
  const diceEmojis = ["\u2680", "\u2681", "\u2682", "\u2683", "\u2684", "\u2685"];
  const descriptionKey = details.tie ? "commands.economy.gamble.dice.tie" : details.won ? "commands.economy.gamble.dice.won" : "commands.economy.gamble.dice.lost";
  return new EmbedBuilder().setTitle(t("commands.economy.gamble.dice.title", { lng: locale })).setDescription(t(descriptionKey, { lng: locale })).setColor(details.won ? 3066993 : details.tie ? 15965202 : 15158332).addFields(
    {
    ...
    },
    {
      ...
      value: result.profit > 0 ? `${t("commands.economy.gamble.dice.resultWon", { defaultValue: "Won", lng: locale })} ${settings.currencySymbol}${result.profit.toLocaleString()}` : result.profit === 0 ? t("commands.economy.gamble.dice.resultPush", { defaultValue: "Push", lng: locale }) : `${t("commands.economy.gamble.dice.resultLost", { defaultValue: "Lost", lng: locale })} ${settings.currencySymbol}${Math.abs(result.profit).toLocaleString()}`,
      inline: true
    },
    {
...
function createCoinflipEmbed(result, settings, locale) {
  const details = result.details;
  return new EmbedBuilder().setTitle(t("commands.economy.gamble.coinflip.title", { lng: locale })).setDescription(
    t("commands.economy.gamble.coinflip.landed", {
      lng: locale,
      result: details.result
    })
  ).setColor(details.won ? 3066993 : 15158332).addFields(
    {
    ...
    {
      name: details.won ? t("commands.economy.gamble.common.wonLabel", { lng: locale }) : t("commands.economy.gamble.common.lostLabel", { lng: locale }),
...
function createSlotsEmbed(result, settings, locale) {
  const details = result.details;
  const embed = new EmbedBuilder().setTitle(t("commands.economy.gamble.slots.title", { lng: locale })).setDescription(`**${details.reels.join(" | ")}**`).setColor(details.won ? 3066993 : 15158332).addFields(
    {
      name: t("commands.economy.gamble.common.result", { lng: locale }),
      value: details.won ? details.winType === "jackpot" ? t("commands.economy.gamble.slots.jackpot", { lng: locale }) : details.winType === "triple" ? t("commands.economy.gamble.slots.triple", { lng: locale }) : t("commands.economy.gamble.slots.double", { lng: locale }) : t("commands.economy.gamble.slots.noMatch", { lng: locale }),
      inline: true
    },
    {
      name: details.won ? t("commands.economy.gamble.common.wonLabel", { lng: locale }) : t("commands.economy.gamble.common.lostLabel", { lng: locale }),
...
function createBlackjackEmbed(result, settings, locale, avatarUrl) {
  const details = result.details;
  const formatHand = (hand) => {
    return hand.cards.map((c) => `${c.rank}${c.suit}`).join(" ");
  };
  const embed = new EmbedBuilder().setTitle(t("commands.economy.gamble.blackjack.title", { lng: locale })).setThumbnail(avatarUrl).setColor(details.won ? 3066993 : details.push ? 15965202 : 15158332).addFields(
    {
...
${t("commands.economy.gamble.blackjack.value", {
      ...
      })}${details.playerHand.blackjack ? t("commands.economy.gamble.blackjack.blackjack", { lng: locale }) : ""}${details.playerHand.bust ? t("commands.economy.gamble.blackjack.bust", { lng: locale }) : ""}`,
      inline: true
    },
    {
...
${t("commands.economy.gamble.blackjack.value", {
      ...
      })}${details.dealerHand.blackjack ? t("commands.economy.gamble.blackjack.blackjack", { lng: locale }) : ""}${details.dealerHand.bust ? t("commands.economy.gamble.blackjack.bust", { lng: locale }) : ""}`,
      inline: true
    },
    {
      name: t("commands.economy.gamble.common.result", { lng: locale }),
      value: details.push ? t("commands.economy.gamble.blackjack.push", { lng: locale }) : details.won ? t("commands.economy.gamble.blackjack.result.win", { lng: locale }) : t("commands.economy.gamble.blackjack.result.loss", { lng: locale }),
      inline: false
    },
    {
      name: details.won ? t("commands.economy.gamble.common.wonLabel", { lng: locale }) : details.push ? t("commands.economy.gamble.blackjack.returned", { lng: locale }) : t("commands.economy.gamble.common.lostLabel", { lng: locale }),
      value: `${settings.currencySymbol}${Math.abs(result.profit).toLocaleString()}`,
      inline: true
    },
    {
...
function createRouletteEmbed(result, settings, locale) {
  const details = result.details;
  const colorEmoji = details.color === "red" ? "\u{1F534}" : details.color === "black" ? "\u26AB" : "\u{1F7E2}";
  const embed = new EmbedBuilder().setTitle(t("commands.economy.gamble.roulette.title", { lng: locale })).setDescription(
