...
export const data = new SlashCommandBuilder().setName("help").setDescription(t("commands.help.description", { defaultValue: "Get help for commands" })).addStringOption(
  (option) => option.setName("command").setDescription(
    t("commands.utils.subcommands.help.options.command", {
      defaultValue: "The command to get help for"
    })
  ).setDescriptionLocalizations(createLocalizationMap(optionDescriptions.command)).setRequired(false).setAutocomplete(true)
);
...
export async function autocomplete(interaction) {
  ...
  const filtered = commands.filter((cmd) => cmd.toLowerCase().includes(focused.toLowerCase())).slice(0, 25);
