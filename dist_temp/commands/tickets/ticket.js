...
function getButtonStyleLabel(style) {
  switch (style) {
...
function buildPanelPreviewEmbed(draft, mode, options) {
  const embed = new EmbedBuilder().setTitle(mode === "create" ? t("tickets.panelCreation") : t("tickets.editingPanel")).setDescription(
    mode === "create" ? t("tickets.configuringPanel", { id: draft.panelId }) : t("tickets.editingPanelDesc", { id: draft.panelId })
  ).setColor(mode === "create" ? 65280 : 5793266).addFields(
  ...
  embed.addFields({
    ...
    value: draft.supportRoles.length > 0 ? draft.supportRoles.map((roleId) => `<@&${roleId}>`).join(", ") : t("tickets.noAdditionalRoles"),
    inline: false
  });
  embed.addFields({
  ...
  if (draft.welcomeMessage) {
    embed.addFields({
      ...
      value: draft.welcomeMessage.length > 256 ? `${draft.welcomeMessage.slice(0, 253)}...` : draft.welcomeMessage,
      inline: false
    });
  }
  return embed;
...
export const ticket = {
  ...
  data: new SlashCommandBuilder().setName("ticket").setDescription(t("commands.ticket.description", { defaultValue: "Manage ticket system" })).setNameLocalizations(createLocalizationMap(commandNames.ticket)).setDescriptionLocalizations(createLocalizationMap(commandDescriptions.ticket)).addSubcommandGroup(
    (group) => group.setName("panel").setDescription(
      ...
      (subcommand) => subcommand.setName("create").setDescription(
        ...
        (option) => option.setName("panel_id").setDescription("Unique ID for this panel").setRequired(true).setMinLength(3).setMaxLength(20)
      ).addStringOption(
        (option) => option.setName("title").setDescription("Panel title").setRequired(true).setMaxLength(256)
      ).addStringOption(
        (option) => option.setName("description").setDescription("Panel description").setRequired(true).setMaxLength(4096)
      ).addStringOption(
        (option) => option.setName("button_label").setDescription("Label for the create ticket button").setMaxLength(80)
      ).addIntegerOption(
        (option) => option.setName("button_style").setDescription("Button style").addChoices(
          {
            name: "Primary (Blue)",
        ...
        (option) => option.setName("category").setDescription("Category to create tickets in").addChannelTypes(ChannelType.GuildCategory)
      ).addRoleOption(
        (option) => option.setName("support_role").setDescription("Support role that can see tickets")
      ).addIntegerOption(
        (option) => option.setName("max_tickets").setDescription("Maximum tickets per user").setMinValue(1).setMaxValue(10)
      ).addStringOption(
        (option) => option.setName("welcome_message").setDescription("Welcome message shown in new tickets").setMaxLength(1024)
      )
    ).addSubcommand(
      (subcommand) => subcommand.setName("load").setDescription(
        ...
        (option) => option.setName("panel_id").setDescription("ID of the panel to load").setRequired(true)
      ).addChannelOption(
        (option) => option.setName("channel").setDescription("Channel to send the panel to").setRequired(true).addChannelTypes(ChannelType.GuildText)
      )
    ).addSubcommand(
      (subcommand) => subcommand.setName("delete").setDescription(
        ...
        (option) => option.setName("panel_id").setDescription("ID of the panel to delete").setRequired(true)
      )
    ).addSubcommand(
      ...
      (subcommand) => subcommand.setName("edit").setDescription(
        ...
        (option) => option.setName("panel_id").setDescription("ID of the panel to edit").setRequired(true)
      )
    ).addSubcommand(
      (subcommand) => subcommand.setName("add_dept").setDescription(
        ...
        (option) => option.setName("panel_id").setDescription("ID of the panel").setRequired(true)
      ).addStringOption(
        (option) => option.setName("dept_id").setDescription("Unique department ID").setRequired(true)
      ).addStringOption(
        (option) => option.setName("name").setDescription("Department name").setRequired(true)
      ).addStringOption(
        (option) => option.setName("description").setDescription("Department description").setRequired(true)
      ).addRoleOption(
        (option) => option.setName("support_role").setDescription("Support role for this department")
      ).addChannelOption(
        (option) => option.setName("category").setDescription("Category for this department").addChannelTypes(ChannelType.GuildCategory)
      ).addStringOption(
        (option) => option.setName("welcome_message").setDescription("Welcome message for this department")
      )
    ).addSubcommand(
      (subcommand) => subcommand.setName("list_depts").setDescription(
      ...
      (subcommand) => subcommand.setName("remove_dept").setDescription(
        ...
        (option) => option.setName("dept_id").setDescription("ID of the department to remove").setRequired(true)
      )
    )
  ).addSubcommand(
    (subcommand) => subcommand.setName("claim").setDescription(
    ...
    (subcommand) => subcommand.setName("close").setDescription(
      t("commands.ticket.subcommands.close.description", { defaultValue: "Close a ticket" })
    ).setDescriptionLocalizations(createLocalizationMap(subcommandDescriptions.ticket.close)).addStringOption(
      (option) => option.setName("reason").setDescription("Reason for closing the ticket").setMaxLength(1e3)
    )
  ).addSubcommand(
    (subcommand) => subcommand.setName("stats").setDescription("View ticket statistics for this server")
...
async function handlePanelCreate(interaction, ticketService, _locale) {
  ...
  const buildComponents = () => {
    const baseRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId("panel_add_roles").setLabel(t("tickets.addMoreRoles")).setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("panel_set_image").setLabel(t("tickets.setImage")).setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("panel_set_footer").setLabel(t("tickets.setFooter")).setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("panel_confirm").setLabel(t("tickets.confirmCreate")).setStyle(ButtonStyle.Success)
    );
    return [baseRow];
  };
    ...
    switch (buttonInteraction.customId) {
      case "panel_confirm":
        try {
          ...
          const successEmbed = new EmbedBuilder().setTitle(t("tickets.panelCreated")).setDescription(t("tickets.panelCreatedDesc", { id: panelId })).setColor(65280).addFields([
            {
      ...
      case "panel_add_roles": {
        ...
        const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.addRolesTitle")).addComponents(
          new ActionRowBuilder().addComponents(
            new TextInputBuilder().setCustomId("roleIds").setLabel(t("tickets.modals.addRolesLabel")).setStyle(TextInputStyle.Paragraph).setPlaceholder(
        ...
        const roleIds = roleIdsInput ? Array.from(
          new Set(
            roleIdsInput.split(/[\s,]+/).map((id) => id.trim()).filter((id) => id.length > 0)
          )
        ) : [];
        if (roleIds.length === 0) {
        ...
        panelDraft.supportRoles = Array.from(/* @__PURE__ */ new Set([...panelDraft.supportRoles, ...roleIds]));
        await modalInteraction.reply({
      ...
      case "panel_set_image": {
        ...
        const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.imageTitle")).addComponents(
          new ActionRowBuilder().addComponents(
            new TextInputBuilder().setCustomId("imageUrl").setLabel(t("tickets.modals.imageLabel")).setStyle(TextInputStyle.Short).setPlaceholder(
      ...
      case "panel_set_footer": {
        ...
        const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.footerTitle")).addComponents(
          new ActionRowBuilder().addComponents(
            new TextInputBuilder().setCustomId("footerText").setLabel(t("tickets.modals.footerLabel")).setStyle(TextInputStyle.Short).setPlaceholder(t("tickets.modals.footerPlaceholder")).setRequired(false).setMaxLength(256)
          )
        );
        await buttonInteraction.showModal(modal);
        let modalInteraction = null;
        ...
        panelDraft.footer = footerText.length > 0 ? footerText : void 0;
        await modalInteraction.reply({
...
async function handlePanelLoad(interaction, _ticketService, _locale) {
  await interaction.deferReply({ ephemeral: true });
  const panelId = interaction.options.getString("panel_id", true);
...
async function handlePanelAddDept(interaction, _locale) {
  await interaction.deferReply({ ephemeral: true });
...
async function handlePanelListDepts(interaction, _locale) {
  await interaction.deferReply({ ephemeral: true });
  const guildId = interaction.guildId;
  const panelCustomId = interaction.options.getString("panel_id", true);
  try {
    ...
    const embed = new EmbedBuilder().setTitle(`Departments for Panel: ${panelCustomId}`).setColor(5793266).setDescription(
...
async function handlePanelRemoveDept(interaction, _locale) {
  await interaction.deferReply({ ephemeral: true });
  const guildId = interaction.guildId;
  const panelCustomId = interaction.options.getString("panel_id", true);
  const departmentId = interaction.options.getString("dept_id", true);
...
async function handlePanelDelete(interaction, ticketService, locale) {
  await interaction.deferReply({ ephemeral: true });
  const panelId = interaction.options.getString("panel_id", true);
  try {
...
async function handlePanelList(interaction, ticketRepository, locale) {
  await interaction.deferReply({ ephemeral: true });
  ...
  const embed = new EmbedBuilder().setTitle(t("tickets.panelList", { lng: locale })).setColor(5793266).setDescription(
    panels.map(
      (panel) => `**${panel.panelId}**
${t("tickets.title", { lng: locale })}: ${panel.title}
${t("tickets.status", { lng: locale })}: ${panel.isActive ? "\u2705" : "\u274C"}
${t("tickets.created", { lng: locale })}: <t:${Math.floor(panel.createdAt.getTime() / 1e3)}:R>`
    ).join("\n\n")
  );
  await interaction.editReply({ embeds: [embed] });
}
async function handlePanelEdit(interaction, ticketService, _locale) {
  await interaction.deferReply({ ephemeral: true });
  const panelId = interaction.options.getString("panel_id", true);
  ...
  const supportRoles = currentPanel.supportRoles || [];
  const panelDraft = {
    panelId: currentPanel.panelId,
    title: currentPanel.title,
    description: currentPanel.description,
    buttonLabel: currentPanel.buttonLabel,
    buttonStyle: currentPanel.buttonStyle ?? ButtonStyle.Primary,
    categoryId: currentPanel.categoryId ?? void 0,
    supportRoles,
    maxTicketsPerUser: currentPanel.maxTicketsPerUser ?? 1,
    welcomeMessage: currentPanel.welcomeMessage ?? void 0,
    imageUrl: currentPanel.imageUrl ?? void 0,
    footer: currentPanel.footer ?? void 0,
    ticketNameFormat: currentPanel.ticketNameFormat ?? void 0
  };
  const selectMenuId = `panel_edit_field_${interaction.id}`;
  const buildComponents = () => {
    const selector = new StringSelectMenuBuilder().setCustomId(selectMenuId).setPlaceholder(t("tickets.selectFieldToEdit")).addOptions(
      {
    ...
    const actionsRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId("panel_edit_toggle").setLabel(currentPanel.isActive ? t("tickets.deactivatePanel") : t("tickets.activatePanel")).setStyle(currentPanel.isActive ? ButtonStyle.Danger : ButtonStyle.Success),
      new ButtonBuilder().setCustomId("panel_edit_close").setLabel(t("tickets.closeEditor")).setStyle(ButtonStyle.Secondary)
    );
  ...
  const awaitModal = async (component, modal, modalId) => {
    await component.showModal(modal);
    try {
      const submission = await component.awaitModalSubmit({
        time: 3e5,
        filter: (interactionFilter) => interactionFilter.customId === modalId && interactionFilter.user.id === interaction.user.id
      });
      return submission;
    } catch (error) {
      if (component.isRepliable()) {
        await (component.deferred || component.replied ? component.followUp({
          content: t("tickets.modals.timeout"),
          ephemeral: true
        }) : component.reply({
          content: t("tickets.modals.timeout"),
    ...
    if (componentInteraction.customId === selectMenuId && componentInteraction.isStringSelectMenu()) {
      ...
      switch (value) {
        case "title": {
          ...
          const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.titleTitle")).addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("value").setLabel(t("tickets.modals.titleLabel")).setStyle(TextInputStyle.Short).setValue(panelDraft.title).setMaxLength(256)
            )
          );
        ...
        case "description": {
          ...
          const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.descriptionTitle")).addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("value").setLabel(t("tickets.modals.descriptionLabel")).setStyle(TextInputStyle.Paragraph).setValue(panelDraft.description).setMaxLength(4096)
            )
          );
          const submission = await awaitModal(componentInteraction, modal, modalId);
          if (!submission) return;
        ...
        case "button_label": {
          ...
          const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.buttonLabelTitle")).addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("value").setLabel(t("tickets.modals.buttonLabel")).setStyle(TextInputStyle.Short).setValue(panelDraft.buttonLabel).setMaxLength(80)
            )
          );
          const submission = await awaitModal(componentInteraction, modal, modalId);
          if (!submission) return;
        ...
        case "button_style": {
          ...
          const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.buttonStyleTitle")).addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("value").setLabel(t("tickets.modals.buttonStyleLabel")).setStyle(TextInputStyle.Short).setPlaceholder(
        ...
        case "support_roles": {
          ...
          const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.supportRolesTitle")).addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("value").setLabel(t("tickets.modals.supportRolesLabel")).setStyle(TextInputStyle.Paragraph).setValue(panelDraft.supportRoles.join("\n")).setRequired(false)
            )
          );
          const submission = await awaitModal(componentInteraction, modal, modalId);
          if (!submission) return;
          const text = submission.fields.getTextInputValue("value").trim();
          const roles = text ? Array.from(
            new Set(
              text.split(/[\s,]+/).map((id) => id.trim()).filter((id) => id.length > 0)
            )
          ) : [];
          const updated = await ticketService.updatePanel(interaction.guild, panelDraft.panelId, {
            supportRoles: roles
          ...
          await submission.reply({
            content: roles.length ? t("tickets.messages.supportRolesUpdated", { count: roles.length }) : t("tickets.messages.supportRolesCleared"),
            ephemeral: true
          });
          await refreshUI();
          break;
        ...
        case "image": {
          ...
          const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.imageTitle")).addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("value").setLabel(t("tickets.modals.imageLabel")).setStyle(TextInputStyle.Short).setValue(panelDraft.imageUrl ?? "").setPlaceholder(
                t("tickets.modals.placeholders.imageUrl", {
                  defaultValue: "https://example.com/panel.png"
                })
              ).setRequired(false)
          ...
          currentPanel = updated;
          panelDraft.imageUrl = url.length > 0 ? url : void 0;
          await submission.reply({
        ...
        case "footer": {
          ...
          const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.footerTitle")).addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("value").setLabel(t("tickets.modals.footerLabel")).setStyle(TextInputStyle.Short).setValue(panelDraft.footer ?? "").setRequired(false).setMaxLength(256)
            )
          );
          const submission = await awaitModal(componentInteraction, modal, modalId);
          if (!submission) return;
          ...
          currentPanel = updated;
          panelDraft.footer = footer.length > 0 ? footer : void 0;
          await submission.reply({
        ...
        case "category": {
          ...
          const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.categoryTitle")).addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("value").setLabel(t("tickets.modals.categoryLabel")).setStyle(TextInputStyle.Short).setValue(panelDraft.categoryId ?? "").setPlaceholder(t("tickets.modals.categoryPlaceholder")).setRequired(false)
            )
          );
          const submission = await awaitModal(componentInteraction, modal, modalId);
          if (!submission) return;
          ...
          currentPanel = updated;
          panelDraft.categoryId = categoryId.length > 0 ? categoryId : void 0;
          await submission.reply({
            content: categoryId.length > 0 ? t("tickets.messages.categoryUpdated", { channel: `<#${categoryId}>` }) : t("tickets.messages.categoryCleared"),
            ephemeral: true
          });
          await refreshUI();
          break;
        ...
        case "max_tickets": {
          ...
          const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.maxTicketsTitle")).addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("value").setLabel(t("tickets.modals.maxTicketsLabel")).setStyle(TextInputStyle.Short).setValue(panelDraft.maxTicketsPerUser.toString()).setRequired(true)
            )
          );
          const submission = await awaitModal(componentInteraction, modal, modalId);
          if (!submission) return;
        ...
        case "welcome_message": {
          ...
          const modal = new ModalBuilder().setCustomId(modalId).setTitle(t("tickets.modals.welcomeTitle")).addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId("value").setLabel(t("tickets.modals.welcomeLabel")).setStyle(TextInputStyle.Paragraph).setValue(panelDraft.welcomeMessage ?? "").setRequired(false).setMaxLength(1024)
            )
          );
          const submission = await awaitModal(componentInteraction, modal, modalId);
          if (!submission) return;
          ...
          currentPanel = updated;
          panelDraft.welcomeMessage = welcome.length > 0 ? welcome : void 0;
          await submission.reply({
            content: welcome.length > 0 ? t("tickets.messages.welcomeUpdated") : t("tickets.messages.welcomeCleared"),
            ephemeral: true
          });
          await refreshUI();
          break;
      ...
      switch (componentInteraction.customId) {
        case "panel_edit_toggle": {
          ...
          await componentInteraction.editReply({
            content: updated.isActive ? t("tickets.messages.panelActivated") : t("tickets.messages.panelDeactivated")
          });
          await refreshUI();
          break;
        }
...
async function handleClaim(interaction, ticketService, ticketRepository, locale) {
  const ticket2 = await ticketRepository.getTicketByChannel(interaction.channelId);
  if (!ticket2) {
    await interaction.reply({
  ...
  try {
    await ticketService.claimTicket(ticket2.id, interaction.member, locale);
    await interaction.reply({
...
async function handleClose(interaction, ticketService, ticketRepository, locale) {
  const ticket2 = await ticketRepository.getTicketByChannel(interaction.channelId);
  if (!ticket2) {
    await interaction.reply({
      content: t("tickets.notInTicket"),
...
async function handleStats(interaction, ticketRepository, locale) {
  await interaction.deferReply({ ephemeral: true });
  const stats = await ticketRepository.getTicketStats(interaction.guildId);
  const embed = new EmbedBuilder().setTitle(t("tickets.statistics", { lng: locale })).setColor(5793266).addFields([
    {
