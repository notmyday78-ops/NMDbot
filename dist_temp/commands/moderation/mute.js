...
export const data = new SlashCommandBuilder().setName("mute").setDescription(t("commands.moderation.subcommands.mute.description")).addUserOption(
  (option) => option.setName("user").setDescription(t("commands.moderation.subcommands.mute.options.user")).setRequired(true)
).addIntegerOption(
  (option) => option.setName("duration").setDescription(t("commands.moderation.subcommands.mute.options.duration")).setRequired(false).setMinValue(1).setMaxValue(10080)
).addStringOption(
  (option) => option.setName("reason").setDescription(t("commands.moderation.subcommands.mute.options.reason")).setRequired(false).setMaxLength(500)
...
export async function execute(interaction) {
  ...
  try {
    ...
    const durationMs = durationMinutes ? durationMinutes * 60 * 1e3 : null;
    ...
    try {
      await user.send({
        embeds: [
          new EmbedBuilder().setColor(8421504).setTitle(t("commands.moderation.subcommands.mute.dmTitle")).setDescription(
    ...
    const embed = new EmbedBuilder().setColor(8421504).setTitle(t("commands.moderation.subcommands.mute.success.title")).setDescription(
