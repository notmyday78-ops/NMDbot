...
export const data = new SlashCommandBuilder().setName("filter").setDescription(t("commands.filter.description")).setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages).addSubcommand(
  (subcommand) => subcommand.setName("add").setDescription(t("commands.filter.add.description")).addStringOption(
    (option) => option.setName("pattern").setDescription(t("commands.filter.add.options.pattern")).setRequired(true)
  ).addStringOption(
    (option) => option.setName("match_type").setDescription(t("commands.filter.add.options.matchType")).addChoices(...MATCH_TYPE_CHOICES)
  ).addBooleanOption(
    (option) => option.setName("case_sensitive").setDescription(t("commands.filter.add.options.caseSensitive"))
  ).addBooleanOption(
    (option) => option.setName("whole_word").setDescription(t("commands.filter.add.options.wholeWord"))
  ).addStringOption(
    (option) => option.setName("severity").setDescription(t("commands.filter.add.options.severity")).addChoices(...SEVERITY_CHOICES)
  ).addBooleanOption(
    (option) => option.setName("auto_delete").setDescription(t("commands.filter.add.options.autoDelete"))
  ).addChannelOption(
    (option) => option.setName("notify_channel").setDescription(t("commands.filter.add.options.notifyChannel")).addChannelTypes(
  ...
  (subcommand) => subcommand.setName("remove").setDescription(t("commands.filter.remove.description")).addIntegerOption(
    (option) => option.setName("rule_id").setDescription(t("commands.filter.remove.options.ruleId")).setRequired(true)
  )
).addSubcommand(
  (subcommand) => subcommand.setName("list").setDescription(t("commands.filter.list.description"))
);
...
async function handleAddRule(interaction) {
  ...
  const matchType = interaction.options.getString("match_type") ?? "literal";
  ...
  const embed = buildRuleEmbed(rule).setColor(5763719).setTitle(t("commands.filter.add.successTitle"));
...
async function handleListRules(interaction) {
  ...
  const embed = new EmbedBuilder().setColor(5793266).setTitle(t("commands.filter.list.title", { count: rules.length })).setDescription(t("commands.filter.list.description")).setTimestamp();
