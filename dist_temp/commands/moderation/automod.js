...
export const data = new SlashCommandBuilder().setName("automod").setDescription("Manage AutoMod V2 rules and Quarantine Vault").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).addSubcommand(
  (subcommand) => subcommand.setName("add_rule").setDescription("Add a new AutoMod V2 rule").addStringOption(
    (option) => option.setName("name").setDescription("Rule name").setRequired(true)
  ).addStringOption(
    (option) => option.setName("trigger_type").setDescription("Type of trigger").setRequired(true).addChoices(
    ...
    (option) => option.setName("action_type").setDescription("Action to take").setRequired(true).addChoices(
    ...
    (option) => option.setName("keywords").setDescription("Comma-separated keywords (for KEYWORD trigger)")
  ).addStringOption(
    (option) => option.setName("regex_pattern").setDescription("Regex pattern (for REGEX trigger)")
  ).addIntegerOption(
    (option) => option.setName("limit").setDescription("Limit threshold (for MENTION_SPAM or ATTACHMENT_SPAM)")
  ).addIntegerOption(
    (option) => option.setName("points").setDescription("Infraction points to add (for ADD_INFRACTION)")
  )
).addSubcommand(
  (subcommand) => subcommand.setName("list_rules").setDescription("List all AutoMod V2 rules")
).addSubcommand(
  (subcommand) => subcommand.setName("remove_rule").setDescription("Remove an AutoMod V2 rule").addStringOption(
    (option) => option.setName("rule_id").setDescription("UUID of the rule to remove").setRequired(true)
  )
).addSubcommand(
  (subcommand) => subcommand.setName("quarantine_list").setDescription("List users currently in the Quarantine Vault")
).addSubcommand(
  (subcommand) => subcommand.setName("quarantine_release").setDescription("Release a user from the Quarantine Vault").addUserOption(
    (option) => option.setName("user").setDescription("User to release").setRequired(true)
  )
);
...
async function handleListRules(interaction) {
  ...
  const embed = new EmbedBuilder().setColor(5793266).setTitle(`AutoMod V2 Rules (${rules.length})`).setDescription(
    rules.map(
      (r) => `\u2022 **${r.name}** (\`${r.id}\`)
...
async function handleRemoveRule(interaction) {
  await interaction.deferReply({ ephemeral: true });
...
async function handleQuarantineList(interaction) {
  await interaction.deferReply({ ephemeral: true });
  ...
  const embed = new EmbedBuilder().setColor(15548997).setTitle(`Quarantine Vault (${list.length})`).setDescription(
    list.map(
      (q) => `\u2022 <@${q.userId}> (\`${q.userId}\`)
  Reason: ${q.reason}
...
async function handleQuarantineRelease(interaction) {
  await interaction.deferReply({ ephemeral: true });
