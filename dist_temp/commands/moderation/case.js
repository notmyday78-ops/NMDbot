...
export const data = new SlashCommandBuilder().setName("case").setDescription(t("commands.moderation.subcommands.case.description")).setDefaultMemberPermissions(PermissionFlagsBits.ModerateMembers);
...
async function handleCaseView(interaction) {
  ...
  const embed = new EmbedBuilder().setColor(10181046).setTitle(t("commands.moderation.subcommands.case.view.title", { id: record.id })).addFields(
    {
  ...
  ).setTimestamp(record.createdAt ?? /* @__PURE__ */ new Date());
  if (record.duration) {
    embed.addFields({
      name: t("commands.moderation.subcommands.case.view.fields.duration"),
      value: formatDuration(Math.round(record.duration / 6e4)),
      inline: true
    });
  }
  if (record.expiresAt) {
    embed.addFields({
      ...
      value: `<t:${Math.floor(new Date(record.expiresAt).getTime() / 1e3)}:f>`,
      inline: true
    });
  }
