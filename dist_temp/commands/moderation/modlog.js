...
export const data = new SlashCommandBuilder().setName("modlog").setDescription(t("commands.moderation.subcommands.modlog.description")).addUserOption(
  (option) => option.setName("user").setDescription(t("commands.moderation.subcommands.modlog.options.user")).setRequired(false)
).addIntegerOption(
  (option) => option.setName("limit").setDescription(t("commands.moderation.subcommands.modlog.options.limit")).setRequired(false).setMinValue(1).setMaxValue(25)
...
export async function execute(interaction) {
  ...
  const cases = targetUser ? await modCaseRepository.getByUserOrModerator(interaction.guild.id, targetUser.id, limit) : await modCaseRepository.getRecent(interaction.guild.id, limit);
  let auditEntries = [];
  try {
    ...
    if (targetUser) {
      auditEntries = entries.filter((entry) => entry.executorId === targetUser.id || entry.targetId === targetUser.id).slice(0, limit);
    } else {
  ...
  if (cases.length === 0 && auditEntries.length === 0) {
    return interaction.editReply({
      content: targetUser ? t("commands.moderation.subcommands.modlog.noUserCases", { user: targetUser.tag }) : t("commands.moderation.subcommands.modlog.noCases")
    });
  }
  const botEmbed = new EmbedBuilder().setColor(9807270).setTitle(
    targetUser ? t("commands.moderation.subcommands.modlog.titleUser", { user: targetUser.tag }) : t("commands.moderation.subcommands.modlog.titleRecent", {
      guild: interaction.guild.name
    })
  ).setDescription(
    cases.length > 0 ? cases.map((record) => {
      const timestamp = Math.floor(new Date(record.createdAt).getTime() / 1e3);
  ...
  const auditEmbed = new EmbedBuilder().setColor(3447003).setTitle(
    targetUser ? t("commands.moderation.subcommands.modlog.titleAuditUser", { user: targetUser.tag }) : t("commands.moderation.subcommands.modlog.titleAuditRecent", {
      guild: interaction.guild.name
    })
  ).setDescription(
    auditEntries.length > 0 ? auditEntries.map((entry) => {
      const timestamp = Math.floor(entry.createdTimestamp / 1e3);
      const actionName = AuditLogEvent[entry.action] ?? `Action (${entry.action})`;
