...
export const data = new SlashCommandBuilder().setName("lock").setDescription(t("commands.moderation.subcommands.lock.description")).addChannelOption(
  (option) => option.setName("channel").setDescription(t("commands.moderation.subcommands.lock.options.channel")).addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement)
).addStringOption(
  (option) => option.setName("reason").setDescription(t("commands.moderation.subcommands.lock.options.reason")).setRequired(false).setMaxLength(500)
...
export async function execute(interaction) {
  ...
  try {
    ...
    const embed = new EmbedBuilder().setColor(15158332).setTitle(t("commands.moderation.subcommands.lock.success.title")).setDescription(
