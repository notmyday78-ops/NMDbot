...
async function handleCommand(interaction) {
  ...
  try {
    ...
    const errorResponse = SecurityErrorHandler.handle(error);
    try {
      if (interaction.deferred && !interaction.replied) {
        await interaction.editReply({
          content: errorResponse.message,
          embeds: errorResponse.embed ? [errorResponse.embed] : void 0
        });
...
async function preDeferInteraction(interaction, ephemeral) {
  if (interaction.deferred || interaction.replied) {
    return;
  }
  try {
...
async function sendSecurityFailureResponse(interaction, message) {
