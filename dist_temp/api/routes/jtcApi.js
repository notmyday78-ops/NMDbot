...
const handlePostConfig = async (req, res) => {
  const { guildId } = getParams(req);
  const {
    baseVoiceChannelId,
    categoryId,
  ...
  try {
    ...
    const embed = new EmbedBuilder().setTitle(panelTitle || "\u{1F50A} Join to Create Voice Channel").setDescription(
      panelDescription || `Join <#${baseVoiceChannelId}> to instantly create your own temporary voice channel!

Use the buttons below to manage your channel's privacy and user limit.`
    ).setColor(5793266).setFooter({ text: "JTC Voice Management" }).setTimestamp();
    const lockBtn = new ButtonBuilder().setCustomId("jtc_lock").setLabel("Lock Channel").setStyle(ButtonStyle.Danger).setEmoji("\u{1F512}");
    const unlockBtn = new ButtonBuilder().setCustomId("jtc_unlock").setLabel("Unlock Channel").setStyle(ButtonStyle.Success).setEmoji("\u{1F513}");
    const limitBtn = new ButtonBuilder().setCustomId("jtc_limit").setLabel("Set User Limit").setStyle(ButtonStyle.Primary).setEmoji("\u{1F465}");
    const row = new ActionRowBuilder().addComponents(lockBtn, unlockBtn, limitBtn);
    if (messageId) {
      try {
    ...
    let updatedConfig;
    if (existingConfig) {
      [updatedConfig] = await db.update(jtcConfigs).set({
        baseVoiceChannelId,
        categoryId,
        panelChannelId,
      ...
      }).where(eq(jtcConfigs.guildId, guildId)).returning();
    } else {
      [updatedConfig] = await db.insert(jtcConfigs).values({
        guildId,
        baseVoiceChannelId,
        categoryId,
        panelChannelId,
...
const handlePanelUpdate = async (req, res) => {
  ...
  try {
    ...
    if (panelChannel && panelChannel.isTextBased()) {
      ...
      const embed = new EmbedBuilder().setTitle(title || "\u{1F50A} Join to Create Voice Channel").setDescription(
        description || `Join <#${config.baseVoiceChannelId}> to instantly create your own temporary voice channel!
...
const handleChannelsLock = async (req, res) => {
  ...
  try {
    const db = getDatabase();
    const [jtcChannel] = await db.select().from(jtcChannels).where(and(eq(jtcChannels.channelId, channelId), eq(jtcChannels.guildId, guildId))).limit(1);
    if (!jtcChannel) {
    ...
    await db.update(jtcChannels).set({ isLocked: true }).where(and(eq(jtcChannels.channelId, channelId), eq(jtcChannels.guildId, guildId)));
...
const handleChannelsUnlock = async (req, res) => {
  const { guildId, channelId } = getParams(req);
  if (!guildId || !channelId) {
    return res.status(400).json({ error: "Bad Request", message: "guildId and channelId are required" });
  }
  try {
    ...
    await db.update(jtcChannels).set({ isLocked: false }).where(and(eq(jtcChannels.channelId, channelId), eq(jtcChannels.guildId, guildId)));
...
const handleChannelsLimit = async (req, res) => {
  const { guildId, channelId } = getParams(req);
  const { userLimit } = req.body;
  if (!guildId || !channelId || userLimit === void 0) {
  ...
  try {
    ...
    await db.update(jtcChannels).set({ userLimit: limit }).where(and(eq(jtcChannels.channelId, channelId), eq(jtcChannels.guildId, guildId)));
