...
router.post("/:guildId/trivia", async (req, res) => {
...
router.get("/:guildId/trivia", async (req, res) => {
  const { guildId } = req.params;
  try {
    const db = getDatabase();
...
router.delete("/:guildId/trivia/:triviaId", async (req, res) => {
  const { guildId, triviaId } = req.params;
  try {
    const db = getDatabase();
    await db.delete(triviaGames).where(and(eq(triviaGames.id, triviaId), eq(triviaGames.guildId, guildId)));
