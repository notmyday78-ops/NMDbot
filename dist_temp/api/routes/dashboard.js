...
router.get("/overview", async (_req, res) => {
  try {
    ...
    if (!stats || statsAggregator.getStatsAge() > 5e3) {
    ...
    const [
      ...
      db.select({
        configured: sql`COUNT(*)`
      }).from(guildSettings).execute().then((rows) => Number(rows?.[0]?.configured ?? 0)),
      db.select({
        total: sql`COUNT(*)`,
        open: sql`COUNT(*) FILTER (WHERE ${tickets.status} = 'open')`,
        closed: sql`COUNT(*) FILTER (WHERE ${tickets.status} = 'closed')`
      }).from(tickets).execute().then((rows) => ({
      ...
      db.select({
        total: sql`COUNT(*)`,
        warnings: sql`COUNT(*) FILTER (WHERE ${modCases.type} = 'warn')`,
        bans: sql`COUNT(*) FILTER (WHERE ${modCases.type} = 'ban')`,
        mutes: sql`COUNT(*) FILTER (WHERE ${modCases.type} = 'mute')`,
        kicks: sql`COUNT(*) FILTER (WHERE ${modCases.type} = 'kick')`
      }).from(modCases).execute().then((rows) => ({
        total: Number(rows?.[0]?.total ?? 0),
      ...
      }).from(tickets).orderBy(desc(tickets.createdAt)).limit(5),
      db.select({
        id: modCases.id,
        guildId: modCases.guildId,
        type: modCases.type,
        createdAt: modCases.createdAt
      }).from(modCases).orderBy(desc(modCases.createdAt)).limit(5),
      db.select({
      ...
      }).from(economyTransactions).orderBy(desc(economyTransactions.createdAt)).limit(5)
    ]);
    let topGuilds = [];
    if (crossShardService.isSharded(client)) {
      const nestedGuilds = await crossShardService.broadcastEval(client, (c) => {
    ...
    topGuilds = topGuilds.sort((a, b) => (b.memberCount ?? 0) - (a.memberCount ?? 0)).slice(0, 5);
    res.json({
      bot: stats.bot,
      guilds: {
        ...stats.guilds,
...
router.get("/guilds", async (req, res) => {
  try {
    ...
    const db = getDatabase();
    const configuredGuilds = await db.select({ guildId: guildSettings.guildId }).from(guildSettings);
...
router.get("/guilds/:guildId/overview", async (req, res) => {
  ...
  try {
    ...
    const [
      ...
      db.select().from(guildsTable).where(eq(guildsTable.id, guildId)).limit(1).then((rows) => rows[0] ?? null),
      db.select().from(guildSettings).where(eq(guildSettings.guildId, guildId)).limit(1).then((rows) => rows[0] ?? null),
      db.select({
        total: sql`COUNT(*)`,
        warnings: sql`COUNT(*) FILTER (WHERE ${modCases.type} = 'warn')`,
        bans: sql`COUNT(*) FILTER (WHERE ${modCases.type} = 'ban')`,
        mutes: sql`COUNT(*) FILTER (WHERE ${modCases.type} = 'mute')`,
        kicks: sql`COUNT(*) FILTER (WHERE ${modCases.type} = 'kick')`
      }).from(modCases).where(eq(modCases.guildId, guildId)).execute().then((rows) => ({
        total: Number(rows?.[0]?.total ?? 0),
        warnings: Number(rows?.[0]?.warnings ?? 0),
        bans: Number(rows?.[0]?.bans ?? 0),
        mutes: Number(rows?.[0]?.mutes ?? 0),
      ...
      db.select({
        ...
        avgResolutionSeconds: sql`
          ...
          `
      }).from(tickets).where(eq(tickets.guildId, guildId)).execute().then((rows) => ({
        total: Number(rows?.[0]?.total ?? 0),
        open: Number(rows?.[0]?.open ?? 0),
        closed: Number(rows?.[0]?.closed ?? 0),
      ...
      db.select({
        ...
        totalBalance: sql`COALESCE(SUM(${economyBalances.balance} + ${economyBalances.bankBalance}), 0)`,
        averageBalance: sql`COALESCE(AVG(${economyBalances.balance} + ${economyBalances.bankBalance}), 0)`
      }).from(economyBalances).where(eq(economyBalances.guildId, guildId)).execute().then((rows) => ({
      ...
      db.select({
        tracked: sql`COUNT(*)`,
        avgLevel: sql`COALESCE(AVG(${userXp.level}), 0)`
      }).from(userXp).where(eq(userXp.guildId, guildId)).execute().then((rows) => ({
        tracked: Number(rows?.[0]?.tracked ?? 0),
        avgLevel: Number(rows?.[0]?.avgLevel ?? 0)
      })),
      db.select({
        count: sql`COUNT(*)`
      }).from(ticketPanels).where(eq(ticketPanels.guildId, guildId)).execute().then((rows) => Number(rows?.[0]?.count ?? 0)),
      db.select({
        id: modCases.id,
        type: modCases.type,
        reason: modCases.reason,
        createdAt: modCases.createdAt
      }).from(modCases).where(eq(modCases.guildId, guildId)).orderBy(desc(modCases.createdAt)).limit(5),
      db.select({
        id: tickets.id,
        status: tickets.status,
        createdAt: tickets.createdAt
      }).from(tickets).where(eq(tickets.guildId, guildId)).orderBy(desc(tickets.createdAt)).limit(5),
      db.select({
        id: economyTransactions.id,
        type: economyTransactions.type,
        amount: economyTransactions.amount,
        createdAt: economyTransactions.createdAt
      }).from(economyTransactions).where(eq(economyTransactions.guildId, guildId)).orderBy(desc(economyTransactions.createdAt)).limit(5),
      db.select({
        activeMembers: sql`COUNT(DISTINCT ${members.userId})`
      }).from(members).where(and(eq(members.guildId, guildId), gte(members.updatedAt, sevenDaysAgo))),
      db.select({
        rewards: sql`COUNT(*)`
      }).from(xpRewards).where(eq(xpRewards.guildId, guildId)).execute().then((rows) => Number(rows?.[0]?.rewards ?? 0))
    ]);
    ...
    res.json({
      guild: {
        ...
        icon: guild.iconURL({ size: 256 }) ?? null,
        memberCount: guild.memberCount,
        approximatePresence: guild.members?.cache ? guild.members.cache.filter(
          (member) => !member.user?.bot && member.presence?.status && member.presence.status !== "offline"
        ).size : 0,
        premiumTier: guild.premiumTier,
        boosters: guild.premiumSubscriptionCount ?? 0,
