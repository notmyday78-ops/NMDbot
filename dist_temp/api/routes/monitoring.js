    ...
    const metrics = {
      ...
      database: {
        ...
        queries: {
          ...
          avgResponseTime: queryMetrics.length > 0 ? `${(queryMetrics.reduce((sum, m) => sum + m.avgTime, 0) / queryMetrics.length).toFixed(2)} ms` : "0 ms"
        },
      ...
      aggregator: {
        ...
        healthy: statsAggregator.getStatsAge() < 2e3
      },
      api: {
...
router.get("/cache", (_req, res) => {
  ...
  res.json({
    ...
    recommendation: stats.hitRate < 50 ? "Consider increasing cache TTL or reviewing cache keys" : "Cache is performing well"
  });
});
...
router.get("/queries", (_req, res) => {
  ...
  res.json({
    ...
    slowQueries: slowQueries.map((q) => ({
      ...
      avgTime: `${q.avgTime.toFixed(2)} ms`,
      count: q.count,
      impact: q.totalTime > 1e3 ? "HIGH" : q.totalTime > 500 ? "MEDIUM" : "LOW"
    })),
    recommendations: generateQueryRecommendations(metrics, slowQueries)
  });
});
router.post("/queries/reset", (_req, res) => {
...
router.get("/rate-limits", (_req, res) => {
  ...
  res.json({
    ...
    recommendation: status.activeKeys > 100 ? "High number of rate-limited IPs. Consider reviewing rate limits or investigating potential abuse." : "Rate limiting is functioning normally"
  });
});
...
function generateQueryRecommendations(metrics, slowQueries) {
