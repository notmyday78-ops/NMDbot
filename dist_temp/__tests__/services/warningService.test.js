...
const mockRepo = warningRepository;
const mockAudit = auditLogger;
const mockEnsure = ensureUserAndGuildExist;
const createWarningRecord = (overrides = {}) => ({
  id: 1,
  warnId: "W123",
  guildId: "guild-1",
  userId: "user-1",
...
const createAutomation = (overrides = {}) => ({
  id: 1,
  automationId: "AUTO1",
  guildId: "guild-1",
  name: "Auto warn",
  ...
  triggerValue: 3,
  actions: [{ type: "timeout", duration: 36e5 }],
  enabled: true,
  createdBy: "mod-1",
...
describe("WarningService", () => {
  ...
  beforeEach(() => {
    ...
    user = createMockUser();
    moderator = createMockUser({ id: "mod-1" });
  });
  afterEach(() => {
    jest.resetModules();
  });
  ...
  describe("editWarning", () => {
    it("updates a warning when it exists", async () => {
      ...
      mockRepo.updateWarning.mockResolvedValueOnce(updated);
    ...
    it("throws when warning does not exist", async () => {
      mockRepo.getWarningById.mockResolvedValueOnce(null);
      await expect(
