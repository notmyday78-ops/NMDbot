...
const mockRepository = economyRepository;
...
describe("EconomyService", () => {
  ...
  beforeEach(() => {
    jest.useFakeTimers();
    jest.setSystemTime(/* @__PURE__ */ new Date("2024-01-01T00:00:00.000Z"));
    jest.clearAllMocks();
  ...
  describe("transferMoney", () => {
    ...
    it("transfers funds between users", async () => {
      ...
      mockRepository.getBalance.mockResolvedValueOnce(fromBalance).mockResolvedValueOnce(fromBalance).mockResolvedValueOnce(toBalance).mockResolvedValue(toBalance);
      mockRepository.addToBalance.mockResolvedValueOnce(deductedBalance).mockResolvedValueOnce(creditedBalance);
      mockRepository.createTransaction.mockResolvedValueOnce(debitTransaction).mockResolvedValueOnce(creditTransaction);
      const result = await service.transferMoney("from-user", "to-user", "guild-1", 200);
      expect(result.success).toBe(true);
  ...
  describe("claimDaily", () => {
    it("returns cooldown message when user is on cooldown", async () => {
      const nextAvailable = new Date(Date.now() + 36e5);
    ...
    it("awards daily reward when available", async () => {
      const balance = createBalance({ balance: 1e3 });
      ...
      mockRepository.addToBalance.mockResolvedValueOnce(updatedBalance);
      mockRepository.createTransaction.mockResolvedValueOnce(transaction);
      mockRepository.setCooldown.mockResolvedValueOnce({});
      const result = await service.claimDaily("user-1", "guild-1");
      expect(result.success).toBe(true);
      expect(result.balance).toEqual(updatedBalance);
  ...
  describe("work", () => {
    it("prevents work when on cooldown", async () => {
      ...
      mockRepository.getCooldown.mockResolvedValueOnce({
        ...
        commandType: "work",
        lastUsed: /* @__PURE__ */ new Date(),
        nextAvailable: new Date(Date.now() + 1e4)
      });
