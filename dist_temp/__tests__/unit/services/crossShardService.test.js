  ...
  beforeEach(() => {
    mockClient = {
      ws: {
        status: 0,
        ping: 35,
        shards: /* @__PURE__ */ new Map([[0, { id: 0, status: 0, ping: 35 }]])
      },
      uptime: 123456,
      guilds: {
        cache: /* @__PURE__ */ new Map([
          [
            "123456789012345678",
            {
              ...
              memberCount: 100,
              ownerId: "999999",
              joinedAt: /* @__PURE__ */ new Date("2024-01-01"),
              banner: null,
              description: "Test guild 1",
              features: /* @__PURE__ */ new Set(["COMMUNITY"]),
              large: false,
              vanityURLCode: null,
              members: {
                cache: /* @__PURE__ */ new Map([
              ...
              channels: { cache: /* @__PURE__ */ new Map([["channel1", { size: 1 }]]) }
            }
          ]
        ])
      },
  ...
  describe("getTotalGuildsCount", () => {
    ...
    it("sums guild counts across shards when sharded", async () => {
      mockClient.shard = {
        count: 2,
        ids: [0],
        fetchClientValues: jest.fn().mockResolvedValue([10, 15])
      };
      const count = await crossShardService.getTotalGuildsCount(mockClient);
  ...
  describe("getTotalUsersCount", () => {
    ...
    it("sums user counts across shards when sharded", async () => {
      mockClient.shard = {
        count: 2,
        ids: [0],
        broadcastEval: jest.fn().mockResolvedValue([100, 200])
      };
      const count = await crossShardService.getTotalUsersCount(mockClient);
