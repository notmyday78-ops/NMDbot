...
const EnvSchema = z.object({
  ...
  // Rate Limiting
  RATE_LIMIT_WINDOW: z.coerce.number().min(1e3).max(3e5).default(6e4),
  RATE_LIMIT_MAX_REQUESTS: z.coerce.number().min(1).max(100).default(10),
  GLOBAL_RATE_LIMIT: z.coerce.number().min(100).max(1e4).default(1e3),
  // External APIs
  ...
  ENABLE_SHARDING: z.enum(["true", "false"]).transform((val) => val === "true").default("false"),
  TOTAL_SHARDS: z.string().default("auto"),
  // Logging & Monitoring
  LOG_LEVEL: z.enum(["debug", "info", "warn", "error"]).default("info"),
  LOG_FILE_PATH: z.string().default("./logs"),
  SENTRY_DSN: z.string().optional().transform((val) => {
    if (!val || val === "your_sentry_dsn_here") return void 0;
    return val;
  }),
  WEBHOOK_ERROR_URL: z.string().optional().transform((val) => {
    if (!val || val === "your_webhook_url") return void 0;
    return val;
  }),
  WEBHOOK_AUDIT_URL: z.string().optional().transform((val) => {
    if (!val || val === "your_webhook_url") return void 0;
    return val;
  }),
  // Web Dashboard
  DASHBOARD_ENABLED: z.enum(["true", "false"]).transform((val) => val === "true").default("false"),
  DASHBOARD_PORT: z.coerce.number().min(1024).max(65535).default(3e3),
  DASHBOARD_SECRET: z.string().optional().transform((val) => {
    if (!val || val === "your_random_secret_here") return void 0;
    return val;
  }),
  DASHBOARD_BASE_URL: z.string().optional().transform((val) => {
    if (!val || val === "https://yourdomain.com") return void 0;
    return val;
  }),
  ...
  TEST_GUILD_ID: z.string().optional().transform((val) => {
    if (!val || val === "your_test_guild_id") return void 0;
    return val;
  }),
  // Economy System
  ECONOMY_STARTING_BALANCE: z.coerce.number().min(0).max(1e6).default(1e3),
  ECONOMY_DAILY_REWARD_MIN: z.coerce.number().min(1).max(1e4).default(100),
  ECONOMY_DAILY_REWARD_MAX: z.coerce.number().min(1).max(1e4).default(500),
  ECONOMY_WORK_COOLDOWN: z.coerce.number().min(6e4).max(864e5).default(36e5),
  ECONOMY_ROB_COOLDOWN: z.coerce.number().min(36e5).max(6048e5).default(864e5),
  // XP System
  ...
  XP_COOLDOWN: z.coerce.number().min(1e4).max(3e5).default(6e4),
  ...
  SPAM_WINDOW: z.coerce.number().min(1e3).max(6e4).default(1e4),
  // Feature Toggles
  ...
  // API Configuration
  ENABLE_API: z.enum(["true", "false"]).transform((val) => val === "true").default("true"),
  API_PORT: z.coerce.number().min(1024).max(65535).default(2e3),
  ...
  API_ALLOWED_ORIGINS: z.string().optional().transform((value) => {
    if (!value) {
      return [];
    }
    const normalize = (origin) => origin.trim();
    try {
