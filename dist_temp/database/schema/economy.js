...
export const economyTransactions = pgTable(
  "economy_transactions",
  {
    ...
    // daily, work, gamble, rob, shop, transfer, admin
    amount: bigint("amount", { mode: "number" }).notNull(),
    description: text("description"),
    metadata: jsonb("metadata"),
    // Additional data like gambling game results, shop item purchased, etc.
    relatedUserId: varchar("related_user_id", { length: 255 }),
    // For transfers and robs
    createdAt: timestamp("created_at").defaultNow().notNull()
  },
  (table) => ({
...
export const economyShopItems = pgTable(
  "economy_shop_items",
  {
    ...
    type: varchar("type", { length: 50 }).notNull(),
    // protection, booster, role, custom
    effectType: varchar("effect_type", { length: 50 }),
    // rob_protection, xp_boost, etc.
    effectValue: jsonb("effect_value"),
    // { duration: 86400, multiplier: 2, roleId: "..." }
...
export const economyGamblingStats = pgTable(
  "economy_gambling_stats",
  {
    ...
    // dice, coinflip, slots, blackjack, roulette
...
export const economySettings = pgTable("economy_settings", {
  ...
  currencySymbol: varchar("currency_symbol", { length: 10 }).notNull().default("\u{1F4B0}"),
  ...
  workCooldown: integer("work_cooldown").notNull().default(3600),
  // seconds
  robEnabled: boolean("rob_enabled").notNull().default(true),
  robMinAmount: bigint("rob_min_amount", { mode: "number" }).notNull().default(100),
  robSuccessRate: integer("rob_success_rate").notNull().default(50),
  // percentage
  robCooldown: integer("rob_cooldown").notNull().default(86400),
  // 24 hours
  robProtectionCost: bigint("rob_protection_cost", { mode: "number" }).notNull().default(1e3),
  robProtectionDuration: integer("rob_protection_duration").notNull().default(86400),
  // 24 hours
  maxBet: bigint("max_bet", { mode: "number" }).notNull().default(1e4),
